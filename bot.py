...
async def phone_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not raw_num.startswith('+'):
        raw_num = '+' + raw_num
    try:
        p = phonenumbers.parse(raw_num, None)
...
async def domain_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args: await update.message.reply_text("/domain example.com"); return
    domain = context.args[0]
    out = []
    try:
        w = whois.whois(domain)
        out.append(f"📅 Whois:\nРег: {escape_md(str(w.registrar))}\nСоздан: {escape_md(str(w.creation_date))}\nИстекает: {escape_md(str(w.expiration_date))}")
    except Exception as e:
...
async def leak_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args: await update.message.reply_text("/leak_phone 79161234567"); return
...
async def leak_phone_full(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args: await update.message.reply_text("/leak_phone_full +79161234567"); return
    phone = re.sub(r'[^0-9]', '', context.args[0])
